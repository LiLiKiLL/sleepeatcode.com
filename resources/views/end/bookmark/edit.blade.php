    ...
    <div class="col-md-12">
        <form class="form-horizontal" action="" role="form" method="post">
                    ...
                    <select class="form-control" name="dir_id">
                      @foreach ($dir_list as $k => $dir_info)
                          ...
                          @foreach ($dir_info['children'] as $key => $child_dir_info)
                              <option value="{{ $child_dir_info['id'] }}" style="margin-left:20px;" selected="<?php if($child_dir_info['id'] == $bookmark_info['dir_id']) { echo "true";} else { echo "false"; } ?>">{{ $child_dir_info['name'] }}</option>
                          @endforeach
                      @endforeach
                    </select>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-md-10">
                    <input type="text" class="form-control" name="name" placeholder="" value="{{ $bookmark_info['name'] }}">
                </div>
            </div>
            <div class="form-group">
                <label for="url" class="col-md-2 control-label">链接</label>
                <div class="col-md-10">
                    <input type="text" class="form-control" name="url" placeholder="" value="{{ $bookmark_info['url'] }}">
                </div>
            </div>
            <div class="form-group">
                <label for="desc" class="col-md-2 control-label">描述</label>
                <div class="col-md-10" id="article-editormd">
                    <textarea class="form-control" name="desc">{{ $bookmark_info['desc'] }}</textarea>
                </div>
            </div>
            <div class="form-group">
